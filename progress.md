...
## 会话：2026-08-07（#57 清理 + 文件名乱码修复）
- 删除上传记录 #57（kwai）的 304 条填反脏数据（备份 dev.db.bak-20260807-2），全库非 ASCII campaign_id 已清零
- 文件名乱码根因：multer/busboy 将 UTF-8 文件名按 latin1 解码 → 新增 `toUtf8Filename()` 修复，覆盖媒体/转化/双文件/商户上传四处（`299e5b5`）
...
## 会话：2026-08-10（期商分析：转化率指标补全 + 日期快捷选项）
- 缺口分析：期商数据只有留资/开户两个转化节点，原页面仅"开户率"一个转化率指标，且无趋势视图、无日期快捷选项
- 后端：merchant/channel 报表新增 `avgConvertDays`（留资→开户加权平均天数，同期群口径）；新增 `GET /merchants/reports/daily-trend`（按日留资/开户/开户率，补齐无数据日期）
- 前端：核心指标新增"平均开户天数"卡片（按开户数加权 + 环比，卡片行改 flex 五等分）；新增"每日留资与开户率趋势"全宽图；期商/渠道明细表各加"平均开户天数"及环比列
- 日期快捷选项：今天/昨天/本周/上周/近7天/近30天/本月/上月（周按周一起点，与平台周报口径一致）
- 已部署（8797baa）：前端 200，新接口 401 鉴权正常（路由已注册）

## 会话：2026-08-10（开户周期报表 + BigInt 崩溃修复）
